<?php
    namespace Core;
    
    error_reporting(E_ALL);
    ini_set('display_errors', 'on');
    
    // Определяем базовый путь проекта
    if (!defined('BASE_PATH')) {
        $scriptPath = dirname($_SERVER['SCRIPT_NAME']);
        define('BASE_PATH', $scriptPath !== '/' ? $scriptPath : '');
    }
    
    require_once dirname(__FILE__) . '/project/config/connection.php';
    
    spl_autoload_register(function($class) {
        preg_match('#(.+)\\\\(.+?)$#', $class, $match);
        
        $nameSpace = str_replace('\\', DIRECTORY_SEPARATOR, strtolower($match[1]));
        $className = $match[2];
        
        $path = dirname(__FILE__) . DIRECTORY_SEPARATOR . $nameSpace . DIRECTORY_SEPARATOR . $className . '.php';
        
        if (file_exists($path)) {
            require_once $path;
            
            if (class_exists($class, false)) {
                return true;
            } else {
                throw new \Exception("Класс $class не найден в файле $path. Проверьте правильность написания имени класса внутри указанного файла.");
            }
        } else {
            throw new \Exception("Для класса $class не найден файл $path. Проверьте наличие файла по указанному пути. Убедитесь, что пространство имен вашего класса совпадает с тем, которое пытается найти фреймворк для данного класса. Например, вы создаете класса модели, но забыли заюзать ее через use. В этом случае вы пытаетесь вызвать класс модели в пространстве имен контроллера, а такого файла нет.");
        }
    });
    
    $routes = require dirname(__FILE__) . '/project/config/routes.php';
    
    // Получаем URI и убираем базовый путь и index.php
    $uri = $_SERVER['REQUEST_URI'];
    
    // Убираем query string (параметры после ?)
    if (($pos = strpos($uri, '?')) !== false) {
        $uri = substr($uri, 0, $pos);
    }
    
    // Убираем базовый путь из URI
    if (BASE_PATH && strpos($uri, BASE_PATH) === 0) {
        $uri = substr($uri, strlen(BASE_PATH));
    }
    
    // Убираем index.php из URI, если он есть
    $uri = preg_replace('#^/index\.php/?$#', '', $uri);
    $uri = preg_replace('#^/index\.php/#', '/', $uri);
    
    // Нормализуем URI: убираем двойные слеши, добавляем слеш в начало если нужно
    $uri = '/' . ltrim($uri, '/');
    
    // Если URI пустой или только слеш, делаем его '/'
    if (empty($uri) || $uri === '/') {
        $uri = '/';
    }
    
    $track = ( new Router )      -> getTrack($routes, $uri);
    $page  = ( new Dispatcher )  -> getPage($track);
    
    echo (new View) -> render($page);
